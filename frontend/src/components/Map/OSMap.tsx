      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition((position) => {
          if (!mapRef.current) {
            ...
            const goodPoints = addressPoints.filter(p => p.pm <= 50).map(p => [p.lat, p.lng, p.pm]);
            const moderatePoints = addressPoints.filter(p => p.pm > 50 && p.pm <= 100).map(p => [p.lat, p.lng, p.pm]);
            const unhealthyPoints = addressPoints.filter(p => p.pm > 100).map(p => [p.lat, p.lng, p.pm]);

            const goodHeat = L.heatLayer(goodPoints, { gradient: { 0.4: 'green' }, radius: 25 }).addTo(mapRef.current);
            const moderateHeat = L.heatLayer(moderatePoints, { gradient: { 0.4: 'orange' }, radius: 25 }).addTo(mapRef.current);
            const unhealthyHeat = L.heatLayer(unhealthyPoints, { gradient: { 0.4: 'red' }, radius: 25 }).addTo(mapRef.current);
          }
        });
      }
    }
