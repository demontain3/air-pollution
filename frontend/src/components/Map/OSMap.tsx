...
// for (let i = 0; i < 50; i++) {
//   aqiData.push({
//     lat: 27.695273 + (Math.random() - 0.5) / 100,
//     lng: 85.272083 + (Math.random() - 0.5) / 100,
//     pm: Math.floor(Math.random() * 200) + 1, // random value between 1 and 200
//   });
// }
      ...
      if (navigator.geolocation) {
        navigator.geolocation.getCurrentPosition((position) => {
          if (!mapRef.current) {
            ...
            const addressPoints = generateData(latitude, longitude, 50)

            addressPoints.forEach((p) => {
              const marker = L.marker([p.lat, p.lng], { opacity: 0 }); //opacity may be set to zero
              marker.bindTooltip(`PM: ${p.pm}`, { className: "my-label", offset: [0, 0] });
